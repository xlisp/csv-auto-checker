...
/**
 * CSV自动对比工具 - 单线程版本 + 字段分组
 * 基于主键的CSV对比，支持科学表示法，按差异字段组合分类
 */
public class CSVComparator {
    ...
    private boolean isScientificNotation(String str) {
        ...
        return trimmed.contains("e") && isNumeric(str);
    }
    ...
    private ComparisonResult sequentialCompare(List<Map<String, String>> data1, List<Map<String, String>> data2) {
        ...
        for (String key : commonKeys) {
            ...
            if (processedCount % 1000 == 0) {
                logger.info("已处理 " + processedCount + " / " + commonKeys.size() + " 行");
            }
        }
     ...
     * 按字段组合进行分组统计
     */
    private Map<String, FieldGroupStats> groupByFieldCombination(ComparisonResult result) {
        Map<String, FieldGroupStats> groupStats = new LinkedHashMap<>();

        for (RowDifference diff : result.differences) {
            // 获取此行差异涉及的所有字段，并排序以确保一致性
            List<String> diffFields = diff.differences.stream()
                .map(fd -> fd.field)
                .sorted()
                .collect(Collectors.toList());

            String groupKey = String.join(", ", diffFields);

            FieldGroupStats stats = groupStats.computeIfAbsent(groupKey, k -> new FieldGroupStats(diffFields));
            stats.rowCount++;
            stats.rows.add(diff);
        }

        logger.info("字段组合分组完成，共 " + groupStats.size() + " 个不同的字段组合");
    ...
    public ComparisonResult compareCSVs(String file1, String file2, String outputHtml)
        ...
        // 按字段组合分组
        Map<String, FieldGroupStats> fieldGroups = groupByFieldCombination(result);
    ...
    private void generateHTMLReport(ComparisonResult result, String file1, String file2, String outputPath)
        ...
        html.append("        .field-group { margin: 20px 0; background-color: white; padding: 20px; border-radius: 5px; border-left: 5px solid #3498db; }\n");
        html.append("        .group-header { background-color: #3498db; color: white; padding: 10px; border-radius: 3px; margin-bottom: 15px; }\n");
        html.append("        .group-stats { background-color: #ecf0f1; padding: 10px; margin: 10px 0; border-radius: 3px; }\n");
        ...
        html.append("        .group-badge { background-color: #9c27b0; color: white; padding: 3px 8px; border-radius: 3px; font-size: 0.9em; margin-left: 10px; }\n");
        ...
        html.append("            <div class=\"stat-box\"><h3>").append(result.fieldGroups.size()).append("</h3><p>字段组合数</p></div>\n");
        html.append("        </div>\n");
        html.append("    </div>\n\n");

        // 按字段组合显示差异
        html.append("    <div class=\"summary\">\n");
        html.append("        <h2>按字段组合分类的差异</h2>\n");

        if (result.fieldGroups.isEmpty()) {
            html.append("        <p class=\"no-differences\">✓ 未发现数据差异</p>\n");
        } else {
            // 按行数降序排列
            List<Map.Entry<String, FieldGroupStats>> sortedGroups = result.fieldGroups.entrySet().stream()
                .sorted((e1, e2) -> Integer.compare(e2.getValue().rowCount, e1.getValue().rowCount))
                .collect(Collectors.toList());

            for (Map.Entry<String, FieldGroupStats> entry : sortedGroups) {
                String groupKey = entry.getKey();
                FieldGroupStats stats = entry.getValue();

                html.append("        <div class=\"field-group\">\n");
                html.append("            <div class=\"group-header\">\n");
                html.append("                <h3>字段组合: ").append(escapeHtml(groupKey)).append("</h3>\n");
                html.append("            </div>\n");
                html.append("            <div class=\"group-stats\">\n");
                html.append("                <strong>影响行数:</strong> ").append(stats.rowCount).append(" 行<br>\n");
                html.append("                <strong>涉及字段:</strong> ").append(stats.fields.size()).append(" 个\n");
                html.append("            </div>\n");

                // 显示前20条差异示例
                int displayLimit = Math.min(20, stats.rows.size());
                html.append("            <h4>差异示例（显示前").append(displayLimit).append("条）:</h4>\n");

                for (int i = 0; i < displayLimit; i++) {
                    RowDifference diff = stats.rows.get(i);
                ...
                if (stats.rows.size() > displayLimit) {
                    html.append("            <p class=\"no-differences\">... 还有 ").append(stats.rows.size() - displayLimit).append(" 条差异未显示</p>\n");
                }
    ...
    public void exportDifferencesCSV(String outputPath) throws IOException {
        ...
        try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(Paths.get(outputPath), StandardCharsets.UTF_8))) {
            writer.println("primary_key,field_combination,field,value_file1,value_file2,is_numeric");
            ...
            for (RowDifference diff : comparisonResults.differences) {
                List<String> diffFields = diff.differences.stream()
                    ...
                    .sorted()
                    .collect(Collectors.toList());
                String fieldCombination = String.join(", ", diffFields);

                for (FieldDifference fieldDiff : diff.differences) {
                    writer.printf("\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",%s%n",
                        escapeCsv(diff.key),
                        escapeCsv(fieldCombination),
    ...
    static class ComparisonResult {
        ...
        Map<String, FieldGroupStats> fieldGroups = new LinkedHashMap<>();
    }
    ...
    static class FieldGroupStats {
        List<String> fields;
        int rowCount = 0;
        List<RowDifference> rows = new ArrayList<>();

        FieldGroupStats(List<String> fields) {
            this.fields = fields;
        }
    }
